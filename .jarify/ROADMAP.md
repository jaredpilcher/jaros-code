...
- **[#23 background runs surface — next product-surface build]** expose the internal runner/daemon infra
  (run_forever, experiment chain) as a product surface: `jcode --bg` submits a request to run in the
  background + `jcode logs/attach/stop <id>` read the Jaros log / detach / cancel — med · tractable
  (infra exists internally, wrap it as CLI surface), different axis; via Jarify (new spec EXT-052). Flips
  row #23 partial→works.
