...
## BREAKTHROUGH (2026-06-29) — the hard class FELL to a FITTING reasoner (Qwen3-4B-Thinking)
After the HARD-CLASS SYNTHESIS (.jarify/EXT-030) mapped the class as root-hard for the 2-3B roster (6 approaches all 0), plateau research found a concrete Jetson-FITTING upgrade: **Qwen3-4B-Thinking-2507** (unsloth GGUF, Q4_K_M 2.5GB, native <think> reasoning, fits the 7.3GB device with headroom at ctx 16384). Trialed on the 6 hard bigbar [fail] tasks via r1_code:
- **task cca32949 (fix last() when __reversed__ is None) => test-gated PASS** — a task the 2-3B roster + ALL 6 prior harness approaches failed.
- tasks 4,5,6 => genuine fail; tasks 2,3 => INCONCLUSIVE (device-limited: ~10 tok/s + runaway thinking timeouts + RAM ~224MB). Conclusive: 1/4.
**VERDICT: the hard class was NOT a pure root-capacity wall — a FITTING reasoner breaks it on the CURRENT hardware.** This vindicates the multi-model thesis end-to-end: a stronger fitting model slots in (registry/profiler/router/rewire, zero rebuild) and earns the class the 2-3B roster couldn't. It also reframes the 6 "failed" approaches: a scaffold is a MULTIPLIER not an adder — on a ~0 base they multiplied to 0; on a capable base (qwen3-4b) or on easier tasks they may amplify (-> tasks #35, #36, untested cross-products).
KEY ENGINEERING NOTES: (1) r1_code max_tokens made env-configurable (R1_MAX_TOKENS) — thinking models truncate + false-fail at the old 3500 cap. (2) qwen3-4b is SLOW (~10 tok/s) -> route the hard class via REQ-6 test-gated ESCALATION (cheap gemma/qwen first, escalate on fail), NOT broadly. (3) HONESTY CAVEAT: public more-itertools commits -> memorization not fully excludable for any model; but the 2-3B roster failed identical tasks = genuine capability gain; test-gate sole arbiter; full count device-limited (1/4 conclusive, not a clean 6/6). ADMITTED: .jaros-data/config/models/qwen3-4b-thinking.json (earned hard-multi-step-repo class).
