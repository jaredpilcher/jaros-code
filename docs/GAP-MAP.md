...
| 3 | **Multi-file edits** | probed | localization HARNESS BUG fixed (711a136 — candidate_files now walks the import closure in isolated runs); test-gated cross-file solve WORKS (gemma 6.8s) BUT patched the CALLER not the root-cause file (symptom vs root — a single-caller test can't catch it). hard multi-step-repo still 0. | root-cause-preferring localization (rank shared deps before callers) + multi-caller tests that expose symptom-patching |
...
**Consequent redirect — the parity gaps are OUTSIDE the single-file middle:** **#3 multi-file** (needs runner ext), the **hard multi-step-repo tail** (measured wall — revisit per ladder), **#7 long-horizon bootstrap→ship** (the Foundry, unbuilt), **#4/#10 LATENCY** (completely UNMEASURED — real dimension where local-2B vs CC-cloud genuinely differ), and **#6 real-world messiness** (shadow-log, unavailable). NEXT concrete step (anti-rut axis switch): **instrument latency #4** (time each command class on the Jetson, p50/p95) — cheapest, orthogonal, no hard-task authoring. Then multi-file runner ext + the Foundry. Bootstrap remaining: solution-store (§9.4), embedder/LoRA/distill probes (§9.5/6, Jetson-only training per [[jaros-code-training-plane]]).
