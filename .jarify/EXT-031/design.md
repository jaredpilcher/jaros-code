...
## MEASURED RESULT #2 (2026-06-29) — MBPP (real headroom): scaffolds DON'T lift medium synthesis either
Ran bare vs decomposition vs experiment-to-understand on n=15 MBPP (gemma, 0 errors). Unlike the HumanEval ceiling, bare has headroom here (53%):
- **bare = 0.533** (8/15)  ·  **decomposition = 0.533** (delta +0.000, CI overlap — NO lift, no harm)  ·  experiment-to-understand = 0.467 (delta -0.067, CI overlap, mis-applied).
COMBINED VERDICT (#36 SETTLED): across easy (HumanEval, ceiling) AND medium-with-headroom (MBPP), the scaffolds do **NOT** lift from-scratch synthesis. Decomposition is exactly neutral on MBPP; on trivial HumanEval it over-engineers (negative); experiment-to-understand is a category-mismatched repair scaffold on synthesis (negative).
**REFINED REFRAME (replaces the owner's 'scaffold = multiplier' with the MEASURED version): a scaffold multiplies WITHIN ITS CLASS. Cross-class it is neutral (decomposition on MBPP = +0.0) to negative (a hard-REPAIR scaffold mis-applied to synthesis, or over-engineering a trivial task).** This was the honest correction of the prior UNVERIFIED assumption "scaffolds lift easier classes" — they do not. The implication is the multi-model/per-class routing thesis, vindicated: keep easy/medium synthesis on the bare/light path; reserve the heavy scaffolds for the hard-repo-repair class they were built for. The one promising untested cell remains **#35: scaffolds x a STRONG base x the RIGHT class** — experiment-to-understand/decomposition ON qwen3-4b-thinking ON the hard-repo class (where both the base is capable AND it is the scaffolds' native class). That is the next high-value measurement.
