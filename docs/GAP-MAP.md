...
Highest **(impact × tractability)** right now: **#2 bounded edits** (real number, active levers, flywheel fuel) and standing up **the instruments** (#1/#4/#7/#10 all `unmeasured`/`probed` — the daily-driver suite + latency budget + Foundry are the scoreboard, and PURSUIT §2 says the instrument comes first). The pursuit's first push is therefore the bootstrap sequence (§9): reconcile → census+tag → **daily-driver suite** → this Gap Map → solution-store → embedder/LoRA/distill probes.
