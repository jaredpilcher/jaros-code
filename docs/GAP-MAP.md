...
- HONEST REMAINING (as of 2026-07-04): the fetch plane was NOT yet wired into the orchestrator/planner; the eval runners didn't yet auto-assert `eval_lock()` around measurement.
- **★ BOTH CLOSED (2026-07-07, EXT-038 REQ-3 `26851d7` + REQ-4 `e12c4e6`) — spec now fully COVERED.** REQ-3: `harness/eval_runner.py::run_task_list` (the shared execution core ~11 eval scripts import) now wraps its task-execution body in `research_guard.eval_lock()` — the eval-leak hard-off is automatic for every current/future eval caller, no longer a per-script discipline to remember; proven by 3 tests showing research is genuinely locked DURING task execution and released after (including after a task exception). REQ-4: `harness/web_research.py::research_context(spec)` — deterministic (never model-judged) library-name scan over a 5-entry curated table (flask/pandas/requests/click/sqlalchemy); on a match, one guarded `fetch()` call; `build_system(..., enable_research=False)` prepends the fetched+fenced result to the PLAN prompt only when explicitly enabled and non-empty — default-off, byte-identical when off (proven by a dedicated regression test), any research failure degrades silently (never breaks a build). 8 new tests. Suite 2432→2440, no regression. **HONEST REMAINING now:** whether research-augmented planning actually LIFTS the real-library-systems accept rate is unmeasured — the wiring is proven, the capability lift is not (queued in ROADMAP.md NEXT).
...
- **HONEST SCOPE of #86 (as of 2026-07-04):** only the ACCEPTANCE-ORACLE half landed, and only for the EMBEDDED (sqlite) datastore. The heart of #86 — actually PROVISIONING real external services (Postgres/Redis/Qdrant/Cassandra) — was NOT built.
- **★ THE HEART OF #86 NOW LANDED (2026-07-07, EXT-039 REQ-2, `1b3395e` Redis + `bc266da` Postgres) — two real-service rungs.** `harness/service_provisioner.py` brings up a REAL service via Docker and independently verifies persistence — never trusting the CLI's stdout — mirroring `datastore_oracle.verify_persistence`'s clean-state→drive→independently-verify→cross-invocation discipline exactly, against a genuinely running service instead of a file. **Redis:** `provision_redis`/`teardown` (`--rm`, mem-capped, Docker-picked loopback port, never leaks a container) + a pure-stdlib RESP-over-socket client (NOT `redis-py`). **Postgres:** `provision_postgres` (`POSTGRES_HOST_AUTH_METHOD=trust` — safe: ephemeral, loopback-only, torn down every run, chosen specifically so the client never needs SCRAM-SHA-256 auth) + a hand-rolled Postgres wire-protocol v3 client over a raw socket (NOT `psycopg2`) — a dual readiness gate (`pg_isready` THEN a real connect via the module's own client) was added live after a genuine race was caught (pg_isready reported ready before the module's own client could connect), then verified 3/3 clean before shipping. Both `verify_*_persistence` functions never trust the CLI-under-test; both catch the LOAD-BEARING hollow-persistence case (a CLI printing "Saved!" that never touches the real service). 19/19 tests pass live (10 Redis + 9 Postgres), zero leaked containers, no new dependency. Suite 2410→2429, no regression. **HONEST REMAINING:** Qdrant/Cassandra (2 of 4 named services) and a live DATASTORE creation-suite task that runs INSIDE the build/suite loop against a provisioned service (today's tests use hand-written CLI fixtures, not a real gemma-built system) remain named, not-yet-built follow-ups.
...
| 18 | **External-tool extensibility protocol (MCP client)** (connect stdio/HTTP tool servers; tools join the toolbelt; the ecosystem standard) | closed(EXT-054) | slice 1 + slice 2 landed: `.jcode/mcp.json` (project) + `~/.jcode/mcp.json` (user, project wins on a name collision) register MCP stdio servers; `harness/mcp_client.py` speaks the stdio JSON-RPC transport (launch with a scrubbed env, `initialize`+`notifications/initialized`, `tools/list`, `tools/call`) with EVERY subprocess read timeout-bounded (background-thread + `queue.Queue` reader, portable to Windows) so a dead/hung/slow server degrades to an honest error and never hangs; `harness/mcp_session.py`'s `MCPSessionManager` keeps each configured server's subprocess ALIVE and REUSES it across calls for the life of the CLI process (a crashed session is evicted + transparently relaunched, one bounded retry, else an honest error), closing every live session cleanly and unconditionally at session end (`JcodeCli.on_stop`) — no leaked subprocess; every `tools/call` still reaches the host ONLY as a `mcp.tool_call` Decision through `harness.coding_loop.Runtime.apply` via the gated `mcp_tool_call_tool.py` (SAME network-egress/destructive/privilege-escalation denylist `shell.exec` uses on the server's LAUNCH command, a denylisted config refused before any subprocess spawns, proven by an explicit test); `/mcp` lists servers+live tools, `/mcp call <server> :: <tool> :: <json-args>` invokes one manually; a plain request ALSO routes to a discovered MCP tool WITHOUT an explicit `/mcp call` — the same small local model that already picks `fix`/`find`/`run`/... can pick a `server::tool`, but only an EXACT match against the LIVE discovered-tool registry is ever built into a Decision (a hallucinated/stale pick conservatively falls through to normal routing, proven never to hijack an ordinary request), reaching the host through the SAME gated path (the can't-escalate invariant proven identically on this path); no config anywhere is a byte-identical no-op (zero model calls, zero subprocesses) | MCP resources/prompts (only tools/list+tools/call implemented); server-initiated notifications; the HTTP/SSE transport (stdio only, no remote server support) |
| 19 | **Subagent authoring surface** (user-defined agents: prompt/tools/model in a markdown file; delegate-with-own-context) | closed(EXT-050) | `.jcode/agents/<name>.md` (project) + `~/.jcode/agents/<name>.md` (user, project wins on a name collision) register a delegatable subagent; `harness/subagents.py` discovers + parses optional frontmatter (`description`/`tools`/`model`) + a system-prompt body, never raising on a missing dir or a malformed file; `JcodeCli._run_subagent` composes the body with a delegated task and routes it through the SAME plain-language chain (`_route_plain`) a typed non-slash request already uses (no second reasoning mechanism), reachable via `/subagent <name> :: <task>` or a deterministic "delegate to `<name>` subagent: `<task>`" phrasing (only fires when `<name>` is actually registered); a subagent's `tools:` allowlist is enforced as a new `tool_allowlist` at `harness.coding_loop.Runtime.apply` — the same gate seam EXT-047/EXT-048 use — consulted ONLY AFTER the hard gate already accepted the Decision, so it can only NARROW what the hard gates permit, never widen past them (proven by an explicit test: an allowlisted-but-denylisted `shell.exec` command is still refused by the hard gate); `/agents` additively lists discovered subagents alongside the existing built-in Python fleet; no `.jcode/agents/` anywhere is a byte-identical no-op | narrow the tool allowlist into `/agent`'s/`/fix`'s own internally-constructed Runtimes (mirrors EXT-049 row #20's identical residual); a "model-invocable when relevant" auto-suggestion mode beyond an explicit `/subagent`/"delegate to X" phrasing; genuinely rewiring to a different SERVED Jetson-fitting model per subagent (EXT-021's job — `model:` here only relabels the request to the same local endpoint) |
| 20 | **Fine-grained checkpoint / rewind** (auto-checkpoint before EACH edit; `/rewind` code, conversation, or both) | closed(EXT-049) | a bounded per-edit checkpoint ring (`harness/checkpoint_ring.py`) is captured at the existing `Runtime.apply` hash-chain seam (EXT-047/EXT-048's seam) for every accepted write/edit Decision; `/checkpoints` lists it, `/rewind <n\|id>` restores prior content THROUGH a real `code.write_file` Decision (gated, root-jailed, hash-chain-logged — never a raw file write); `/undo` (EXT-009) unchanged | wire the ring into `/agent`'s internal `Runtime()` construction sites for full-run per-edit granularity; a delete-file Decision type for true un-create (today a file's creation can't be fully undone) |
| 21 | **Interrupt + steer mid-run** (Esc to stop safely mid-task, queue a correction, agent adjusts) | probed(EXT-055) | a cooperative `InterruptController` (`harness/interrupt.py`, a plain threadsafe flag, never a signal-raised exception) is polled at SAFE iteration boundaries — before the next candidate file in `multi_file.py::multi_file_fix`'s cumulative fix loop, and before the next function in `spec_loop.py`'s `_decompose_build`/`_build_per_function` — never mid-write/mid-Decision; on cancel the loop stops gracefully and returns its current partial result with an honest "interrupted after N step(s)" note (no new persistence path — the existing EXT-049 ring/EXT-009 snapshot already covers what's kept). The REPL (`harness/cli.py::_run_command_interruptible`) wires a real Ctrl-C during a RUNNING command to `request_cancel()` instead of an uncaught `KeyboardInterrupt` tearing through and killing the session (today's actual prior behavior); it then reports the interrupt and returns to the prompt, where typing a new instruction is the "steer" half (an ordinary next turn, no special amend mode). DEFAULT (no interrupt requested) is byte-identical for every existing caller. Honestly `partial`: only `/fixrepo`/`/agent`'s FIX+BUILD-per-function flows got cooperative checks; `/buildsystem`'s system-build loop and other long-running commands have no safe point yet, and there is no explicit in-flight-plan amend UX | cooperative cancel points in `/buildsystem`'s (harness/system_builder.py) module-by-module build loop; a genuine amend/steer mode for an in-flight multi-step plan |
| 22 | **Context management for long sessions** (auto-compact, context meter, `@file` references, `/compact`) | closed(EXT-051) | `@path`/`@dir/` references in ANY plain request (typed, or a skill-substituted template) deterministically inline a file's content (bounded, truncation noted) or a bounded directory listing — `harness/atrefs.py` is pure string composition, reading through the EXISTING gated `fs.read`/`fs.list` tools (the same seam `/read`/`/ls` already use, never a raw `open()`), wired into the SAME `_route_plain` chain a typed request and a skill template already share; a missing/unreadable ref degrades to an honest `(not found)` annotation, never a crash. `/compact` durably shrinks the CURRENT session's transcript by reusing the EXISTING `_summarize_turns()` narrow-model-call mechanism EXT-036 REQ-15's `condense()` already built (not a second summarizer) — unlike `condense()`'s transient per-turn VIEW, it mutates + persists `session.turns`, reporting before/after turn+char counts; an already-short session is an honest no-op | a context-usage meter/percentage display; an auto-compact-on-threshold trigger (today's auto-condensation stays a transient view, `/compact` is manual-only); `@path` expansion inside `JCODE.md` itself (already named deferred on row #14, EXT-042) |
| 23 | **Background runs surface** (`--bg`, attach/logs/stop, agent view — kick off a long build, keep working) | closed(EXT-052) | `jcode --bg "<request>"` submits to run DETACHED (a real `subprocess.Popen` worker, `harness/bg_worker.py`) and returns a short job id immediately; the worker's unit of work is the UNCHANGED EXT-043 `_run_one_shot` path, so any host-project write the job performs still goes through the real gated `code.write_file` Decision exactly like a foreground run; a durable `JobRecord` persists under `.jaros-data/bg_jobs/` (internal runtime state); `jcode jobs` (alias `jcode bg list`) lists jobs, `jcode logs <id>` prints output, `jcode attach <id>` streams new output until the job ends or Ctrl-C detaches (never stopping it), `jcode stop <id>` kills only the job's recorded pid/tree (mirrors `harness.secure_exec._kill_tree`, never by name); `/jobs`/`/logs <id>`/`/stop <id>` mirror it in the REPL | a REPL `/attach` (CLI-only in this pass); job resource limits/quotas; a fully-namespaced `jcode bg <verb>` family beyond the `bg list` alias |
...
| 25 | **Install + health story** (one-command install macOS/Linux/Windows, auto-update, `/doctor` diagnostics) | closed(EXT-053) | `harness/doctor.py` runs a deterministic check battery — Python version, git present + inside a work tree, docker present (WARN not FAIL when absent — only needed for some eval paths), `.jaros-data/` writability (a read-only `os.access` permission query, never a write) + its `tools`/`agents` subdirs, `JCODE_LLM_BACKEND` config sanity, and a BOUNDED probe of the Jetson llama.cpp endpoint reusing the existing `harness.llamacpp_client.health` helper (unreachable degrades to an honest WARN, never a hang or a raise, so `/doctor` works fully offline) — with an honest overall pass/warn/fail verdict; wired as `/doctor` (REPL) and `jcode doctor` / `jcode --doctor` (headless, deterministic exit code), `/help` updated; a minimal `pyproject.toml` adds a `jcode` console-script entry point (`pipx install .` / `pip install -e .`) alongside the unchanged `python -m harness.cli` / `serve.sh`/`.ps1` / `jcode.sh`/`.ps1` paths | auto-update; a signed/versioned release artifact; an actual PyPI publish step (only a local editable/pipx install from a checkout is proven here); richer on-device Jetson diagnostics (CUDA/JetPack/llama.cpp build specifics) are out of scope for a host-side `/doctor` |
