...
| 7 | **Long-horizon tasks (bootstrap→ship)** | probed | Foundry stood up + ship-log live (#8). First project `foundry_stats_cli`: built CORRECT core logic (function oracle PASS, 9.6s) but **SHIP=False** — ran as a CLI → EMPTY output. KEY FINDING: `build_from_intent` gates on the FUNCTION oracle, not the PROGRAM RUNNING → correct functions, non-runnable programs (the `__main__`/entry-point is unverified so not reliably built). Fixable HARNESS gap, not a model wall. | **ship-gated build loop**: make the acceptance = run-as-program (feed the CLI-run failure back into the build iteration) → then scale to multi-file projects |
