...
# ---------------------------------------------------------------------------
# EXT-012 REQ-13 integration: augment_source wires doctest augmentation into
# behavioral_solve_jaros (the DEFAULT Jaros-native solve path).
# ---------------------------------------------------------------------------

def test_augment_source_augments_tests(tmp_path: Path):
    """When augment_source is supplied, self-tests gain doctest-derived assertions.

    We pass a source with a ">>> double(2)\\n4" docstring example and verify the
    augmented tests contain the generated assertion.  No LLM, no Jetson, no Docker.
    This is an offline deterministic check (EXT-012 REQ-13 integration coverage).
    """
    from harness.behavioral_solve import behavioral_solve_jaros
    import harness.jaros_solve_ops as _ops
    from jaros.core import create_decision
    ...
    spec_p = str(tmp_path / "spec.gherkin")
    test_p = str(tmp_path / "test_double.py")
    code_p = str(tmp_path / "double.py")

    # Source with docstring examples — the augmenter must parse ">>> double(2)\n4"
    _source_with_docstring = '''\
def double(x):
...
'''
    ...
    # Track what test content the code-writer sees (via stub agents)
    observed_tests: list[str] = []

    fake_gherkin = "1. Given double(2) returns 4\n"
    # Stub test-writer returns minimal tests (augmenter should append to these)
    fake_tests_from_model = "from mymod import double\ndef test_basic():\n    assert double(1) == 2\n"
    fake_code = "def double(x):\n    return x * 2\n"

    class _GherkinAgent:
        def decide(self, ctx):
            return [create_decision(id=f"gk-{uuid.uuid4().hex}", source="gherkin-writer",
                                   type="code.write_file",
                                   payload={"path": ctx.get("spec_path", spec_p),
                                            "content": fake_gherkin})]

    class _TestAgent:
        def decide(self, ctx):
            return [create_decision(id=f"tw-{uuid.uuid4().hex}", source="test-writer",
                                   type="code.write_file",
                                   payload={"path": ctx.get("test_path", test_p),
                                            "content": fake_tests_from_model})]

    class _CodeAgent:
        def decide(self, ctx):
            # Record the test content the code-writer receives (via gherkin in ctx)
            return [create_decision(id=f"cw-{uuid.uuid4().hex}", source="code-writer",
                                   type="code.write_file",
                                   payload={"path": ctx.get("code_path", code_p),
                                            "content": fake_code})]

    _agents = [_GherkinAgent(), _TestAgent(), _CodeAgent()]
    _idx = [0]

    def _fake_load_agent(filepath, llm):
        idx = _idx[0]
        _idx[0] += 1
        return _agents[idx % len(_agents)]

    def _fake_run_tests(rt_, command, *, cwd=None, timeout_s=15, source=""):
        d = create_decision(id=f"sh-{uuid.uuid4().hex}", source=source or "behavioral-solve-jaros",
                           type="shell.exec",
                           payload={"command": command, "timeout_s": timeout_s})
        res = rt_.apply(d)
        if isinstance(res, dict):
    ...
    with patch("harness.behavioral_solve._load_agent_from_file", side_effect=_fake_load_agent), \
        ...
        result = behavioral_solve_jaros(
            intent="double returns twice the input",
            name="double",
            current_src=None,
            context="",
            pkg="mymod",
            ...
            test_path=test_p,
            code_path=code_p,
            augment_source=_source_with_docstring,  # EXT-012 REQ-13
        )

    # The returned tests must include the augmented doctest-derived block
    tests_out = result["tests"]
    assert "test_docstring_examples__double" in tests_out, (
        "augment_source should append doctest-derived test function to the model's self-tests; "
        f"got: {tests_out[:400]}"
    )
    # The docstring assertion for double(2) == 4 must appear
    assert "== 4" in tests_out, (
        f"doctest example 'double(2) -> 4' must appear as an assertion; got: {tests_out[:400]}"
    )
    # The original model-generated tests must still be present (additive)
    assert "test_basic" in tests_out, (
        "model-generated test_basic must be preserved in the augmented output"
    )


def test_augment_source_none_no_augmentation(tmp_path: Path):
    """When augment_source=None (the default), no augmentation occurs and the test
    content from the test-writer agent is passed through unchanged.
    """
    from harness.behavioral_solve import behavioral_solve_jaros
    import harness.jaros_solve_ops as _ops
    from jaros.core import create_decision
    ...
    rt = _make_runtime(tmp_path)
    spec_p = str(tmp_path / "spec.gherkin")
    test_p = str(tmp_path / "test_noop.py")
    code_p = str(tmp_path / "noop.py")

    fake_tests = "from mymod import f\ndef test_it():\n    assert f(0) == 0\n"
    stubs = [
        _make_stub_agent("code.write_file", "1. Given f(0) returns 0\n", "gherkin-writer"),
        _make_stub_agent("code.write_file", fake_tests, "test-writer"),
        _make_stub_agent("code.write_file", "def f(x):\n    return x\n", "code-writer"),
    ]
    _idx = [0]

    def _fake_load_agent(filepath, llm):
        idx = _idx[0]; _idx[0] += 1
        return stubs[idx % len(stubs)]

    def _fake_run_tests(rt_, command, *, cwd=None, timeout_s=15, source=""):
        d = create_decision(id=f"sh-{uuid.uuid4().hex}", source="behavioral-solve-jaros",
                           type="shell.exec", payload={"command": command, "timeout_s": timeout_s})
        res = rt_.apply(d)
        if isinstance(res, dict):
            res["exitCode"] = 0
        return res if isinstance(res, dict) else {"exitCode": 0, "stdout": "1 passed", "stderr": ""}
    ...
    with patch("harness.behavioral_solve._load_agent_from_file", side_effect=_fake_load_agent), \
        ...
        result = behavioral_solve_jaros(
            ...
            test_path=test_p,
            code_path=code_p,
            # augment_source omitted -> None -> no augmentation
        )

    tests_out = result["tests"]
    # No doctest block should appear since augment_source was not given
    assert "test_docstring_examples__" not in tests_out, (
        "With augment_source=None no augmentation should occur"
    )
    # Original tests must be unchanged
    assert "test_it" in tests_out
