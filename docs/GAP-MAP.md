...
| 18 | **External-tool extensibility protocol (MCP client)** (connect stdio/HTTP tool servers; tools join the toolbelt; the ecosystem standard) | probed(EXT-054) | first slice landed: `.jcode/mcp.json` (project) + `~/.jcode/mcp.json` (user, project wins on a name collision) register MCP stdio servers; `harness/mcp_client.py` speaks the stdio JSON-RPC transport (launch with a scrubbed env, `initialize`+`notifications/initialized`, `tools/list`, `tools/call`) with EVERY subprocess read timeout-bounded (background-thread + `queue.Queue` reader, portable to Windows) so a dead/hung/slow server degrades to an honest error and never hangs, and clean subprocess shutdown every time; every `tools/call` reaches the host ONLY as a new `mcp.tool_call` Decision through `harness.coding_loop.Runtime.apply` via a new gated tool (`mcp_tool_call_tool.py`) whose `validate()` applies the SAME network-egress/destructive/privilege-escalation denylist `shell.exec` uses to the server's LAUNCH command (a denylisted server config is refused before any subprocess spawns, proven by an explicit test mirroring EXT-050's identical proof); `/mcp` lists servers+live tools, `/mcp call <server> :: <tool> :: <json-args>` invokes one gated; no config anywhere is a byte-identical no-op | MCP resources/prompts (only tools/list+tools/call implemented); server-initiated notifications; the HTTP/SSE transport (stdio only); a persistent server connection across turns (today launches fresh per call); a model-invocable auto-suggestion mode beyond an explicit `/mcp call` |
