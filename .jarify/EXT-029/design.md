...
## COLLABORATION PROBE VERDICT (2026-06-29) — draft->critique->revise = 0/6
The owner's collaboration hunch, tested in its cheapest form (qwen DRAFT -> gemma CRITIQUE -> qwen REVISE, batched swaps, _run_nodes oracle as SOLE arbiter) on the 6 hard bigbar [fail] tasks gemma+qwen both fail 0/8 ALONE: **0/6 cracked**. The mechanism ran cleanly (all phases, no errors); collaboration did not crack a task that drafting-alone failed.
This is the **5th distinct approach to return 0** on this hard class: pass@k sampling (0/7), decomposition (0/8), maximal-help (0), decorrelated reasoner R1-1.5B (0/6), and now cross-model collaboration (0/6). Accumulating evidence: the hard multi-step-repo class needs a capability the Jetson-fitting small models lack at the ROOT; combining/sampling/decomposing does NOT supply it — failures are correlated at the root, not complementary.
HONEST CAVEATS: (a) this tested ONE collaboration form (draft->critique->revise); the richer multi-turn team-discussion (#33) is untested — but given the cheap structured form showed zero lift, #33 (more turns of the same two root-correlated models) is lower-probability. (b) the critique-quality transcript was not logged; a weak critic could in principle mask a real lever — but 4 prior independent 0s make a thin-implementation explanation less likely than a genuine root-capability wall.
NEXT LEVER (genuinely DIFFERENT kind): #30 experiment-to-understand — an agentic experiment->observe->understand->solve loop that changes the SOLVE PROCESS (build repo-behavior understanding via run-and-observe), not the model combination. This is how strong coding agents (Claude Code, SWE-agent) crack hard repo tasks; the untested lever most likely to address a root-understanding gap.
