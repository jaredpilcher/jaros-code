...
## Deterministic anchor run-2 (2026-06-28) — #20 parity CONFIRMED
2nd DETERMINISTIC fix-loop run on the 101-task held-out bar: **18/101 = 17.8%** [Wilson 11.6-26.4%] (bigbar_jaros2.txt). Deterministic is now 2 runs: **19/101 (run-1) + 18/101 (run-2) = ~18-19/101 with ±1 run-to-run variance** (llama.cpp not byte-deterministic at temp=0). The #20 AGENTIC orchestrator's **17/101** sits WITHIN this deterministic run-to-run noise band -> **#20 ~PARITY CONFIRMED**: the non-deterministic 2B judge ~= the deterministic fix-loop on the honest bar (deterministic marginally ahead, within noise). Honest harness baseline on the 101-task held-out repo bar: **~18/101 ≈ 18%**. Next lever (PRIME intent): GROUND the orchestrator's judgment (#12 deterministic tool+validation library + #21 tool-use-judgment eval), not trust the raw 2B judge.

## #21 JUDGMENT EVAL — 2B judge diagnostic (2026-06-28, EXT-016, honest)
Ran the 10-scenario tool-use-judgment eval on the live 2B judge (_judge_revision, temp=0): **8/10 = 80% accuracy**.
Per-class: syntax->repair 2/2, logic->code 3/3, import->repair 2/2, all_pass->done 1/1, **bad_tests->gherkin 0/2 (FAIL: got code/repair)**.
KEY DIAGNOSIS: the 2B judge correctly recognizes syntax/logic/import/done (8/8) but has ONE blind spot — it does NOT recognize when the SELF-TESTS THEMSELVES are wrong (should pick 'gherkin' to re-derive the spec; instead picks code/repair).
WHY THIS EXPLAINS #20 ~PARITY: the judge's CORRECT decisions (syntax/logic/import) are ones the deterministic fix-loop's fixed code->repair order ALSO handles -> the judge adds no value there; and the ONE decision that WOULD differentiate the orchestrator (bad_tests -> gherkin, which the deterministic loop NEVER does) is exactly the one the judge gets WRONG. So the orchestrator can't beat deterministic.
#12 GROUNDING LEVER (now concrete + testable): deterministically DETECT the bad_tests case (self-tests contradict the visible docstring/intent, OR the test file fails to parse) and route to 'gherkin' (re-derive the spec). Grounding the judge's single blind spot is the path for the orchestrator to BEAT the deterministic fix-loop on the honest 101-bar. Raschka-consistent: ground the weak judgment, don't trust it.
