...
| 2 | **Bounded single-file edits** | in-progress | SWE-bench-Lite CURATED easy-slice **5/8**; HumanEval-40 code-gen gemma 33/40, qwen 36/40. UNCURATED-Lite growth STARTED (2026-07-01): growth pipeline validated (auto build-image→extract→solve→eval, grow_one.sh); django-14534 = qwen applicable-but-wrong (resolved=False) — the easy-8 were curated EASY; uncurated Lite is much harder (the real parity gap). Batch of 3 more running for an honest uncurated rate. | grow the uncurated rate (batch) → localization/repair probes on misses → LoRA S/R emitter (L7) |
